...
public class ProPlan extends AIModel {
    ...
    /**
     * Purchases additional team member slots, increasing the available
     * slot count. Rejects zero or negative values with an error message.
     *
     * @param slots number of additional slots to purchase; must be positive
     * @return confirmation message on success, or an error message if
     *         {@code slots <= 0}
     */
    public String purchaseTeamSlots(int slots) {
        if (slots <= 0) {
            return "Error: Please enter a positive value for additional team slots!";
        }
        teamSlots += slots;
        return "Success! " + slots + " team slot(s) purchased. " +
                "Current available slots: " + teamSlots + ".";
    }
