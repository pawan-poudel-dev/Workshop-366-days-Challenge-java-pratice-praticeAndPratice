...
public class SubscriptionGUI extends JFrame implements ActionListener {
    ...
    private JTextField txtExtraSlots; // Field 11 extra team slots to purchase
    ...
    private JButton btnClear; // clears input fields only
    private JButton btnClearOutput; // clears output area only
    private JButton btnClearAll; // clears fields + output area
    private JButton btnGivePrompt;
    private JButton btnBuyPrompts;
    private JButton btnAddMember;
    private JButton btnRemoveMember;
    private JButton btnBuySlots;
    ...
    private JPanel buildBody() {
        ...
        lScroll.setPreferredSize(new Dimension(490, 0));
    ...
    private JPanel buildAddPlansSection() {
        JPanel card = makeCard("  Plan Details");

        // 4 shared model fields in a tight 4-row grid
        JPanel grid = new JPanel(new GridLayout(4, 2, 5, 3));
        grid.setOpaque(false);
        grid.add(lbl("Model Name"));
        txtModelName = field("e.g. GPT-4o");
        grid.add(txtModelName);
        grid.add(lbl("Price (NPR / 1L tokens)"));
        txtPrice = field("e.g. 250.00");
        grid.add(txtPrice);
        grid.add(lbl("Parameters (Billions)"));
        txtParams = field("e.g. 70");
        grid.add(txtParams);
        grid.add(lbl("Context Window (K)"));
        txtContext = field("e.g. 128");
        grid.add(txtContext);

        // Quota + Slots on one row
        JPanel qRow = new JPanel(new GridLayout(1, 4, 5, 0));
        ...
        // Add Personal | Add Pro | Display All — 3 similar "plan-level" actions
        JPanel btns = new JPanel(new GridLayout(1, 3, 5, 0));
        btns.setOpaque(false);
        btnAddPersonal = btn("Add Personal");
    ...
    private JPanel buildPersonalSection() {
        JPanel card = makeCard("  Personal Plan");

        // Prompt Text + Response Length
        JPanel fields = new JPanel(new GridLayout(2, 2, 5, 3));
        fields.setOpaque(false);
        fields.add(lbl("Prompt Text"));
        txtPromptText = field("Enter prompt...");
        fields.add(txtPromptText);
        fields.add(lbl("Response Length (tokens)"));
        txtResponseLen = field("e.g. 1024");
        fields.add(txtResponseLen);

        // Give Prompt | Buy Prompts | Check Type — 3 personal-plan actions
        JPanel btns = new JPanel(new GridLayout(1, 3, 5, 0));
        btns.setOpaque(false);
    ...
    private JPanel buildProSection() {
        JPanel card = makeCard("  Pro Plan \u2014 Team");

        // Member Name + Extra Slots on one row
        JPanel fields = new JPanel(new GridLayout(2, 2, 5, 3));
        fields.setOpaque(false);
        fields.add(lbl("Team Member Name"));
        txtMemberName = field("e.g. Sita Rai");
        fields.add(txtMemberName);
        fields.add(lbl("Extra Slots to Purchase"));
        txtExtraSlots = field("e.g. 5");
        fields.add(txtExtraSlots);

        // Add Member | Remove Member | Buy Slots — 3 team actions
        JPanel btns = new JPanel(new GridLayout(1, 3, 5, 0));
        btns.setOpaque(false);
        btnAddMember = btn("Add Member");
        btnRemoveMember = btn("Remove Member");
        btnBuySlots = btn("Buy Slots");
        btns.add(btnAddMember);
        btns.add(btnRemoveMember);
        btns.add(btnBuySlots);
    ...
    private JPanel buildUtilitySection() {
        ...
        // Index field
        JPanel idxRow = new JPanel(new GridLayout(1, 2, 5, 0));
        ...
        // Clear Fields | Clear Output | Clear All — 3 clear actions
        ...
        r1.add(btnClear);
        r1.add(btnClearOutput);
        r1.add(btnClearAll);

        // Export | Load — 2 file actions
        JPanel r2 = new JPanel(new GridLayout(1, 2, 5, 0));
        r2.setOpaque(false);
        btnExport = btn("Export File");
        btnLoad = btn("Load File");
        r2.add(btnExport);
        r2.add(btnLoad);
                ...
                BorderFactory.createCompoundBorder(
                        ...
                        BorderFactory.createCompoundBorder(
                                BorderFactory.createTitledBorder(
                                        ...
                                        TitledBorder.TOP,
                                        F_SEC, CA),
                                BorderFactory.createEmptyBorder(0, 10, 7, 10)))));
        return card;
    }
    ...
    /**
     * Resets every input text field to its placeholder state.
     * Called by both "Clear Fields" and "Clear All" buttons so the
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (src == btnAddPersonal) {
            ...
            // Clear Fields only (input fields, NOT output area)
        } else if (src == btnClear) {
            clearFields();
            displayArea.setText("  Input fields cleared.\n");

            // Clear Output only (display area, NOT input fields)
        } else if (src == btnClearOutput) {
            displayArea.setText("  Output cleared.\n");

            // Clear All (input fields + output area)
        } else if (src == btnClearAll) {
            clearFields();
            displayArea.setText("  All cleared.\n");
            ...
            // Buy Team Slots (ProPlan only)
        } else if (src == btnBuySlots) {
            int idx = getDisplayNumber();
            if (idx != -1) {
                AIModel m = plans.get(idx);
                if (m instanceof ProPlan) {
                    try {
                        int extra = Integer.parseInt(safeRead(txtExtraSlots));
                        displayArea.setText("  " + ((ProPlan) m).purchaseTeamSlots(extra));
                    } catch (NumberFormatException ex) {
                        JOptionPane.showMessageDialog(this,
                                "Enter a valid number in the Extra Slots field.",
                                "Input Error", JOptionPane.ERROR_MESSAGE);
                    }
                } else {
                    JOptionPane.showMessageDialog(this,
                            "Buy Team Slots is only available for Pro Plan subscriptions.",
                            "Wrong Plan Type", JOptionPane.ERROR_MESSAGE);
                }
            }
